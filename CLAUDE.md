...
- Card/hero thumbnail resolution (`post-card.ejs`, `pinned-post.ejs`): `cover_image: false` front-matter (strict `=== false`, bare boolean — same convention as `ai_assisted`) suppresses the thumbnail for one post; `cover.fallback: false` theme config disables the `cover.default` fallback for all cards. The `og:image` chain in `head.ejs` deliberately keeps its `cover.default` fallback in both cases — the opt-outs mean "no thumbnail", not "no social-preview image". Do not add the opt-out checks to `head.ejs`.
