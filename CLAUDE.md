...
- Card/hero thumbnail resolution is the `resolve_cover(post)` helper in `scripts/helpers.js` — consumed by `post-card.ejs` and `pinned-post.ejs`; never re-inline the chain in a template. `cover_image: false` front-matter (strict `=== false`, bare boolean — same convention as `ai_assisted`) suppresses the thumbnail for one post; `cover.fallback: false` theme config disables the `cover.default` fallback for all cards (the string `"false"` is also accepted — silently keeping the fallback would invert the user's intent). The `og:image` chain in `head.ejs` deliberately keeps its `cover.default` fallback in both cases — the opt-outs mean "no thumbnail", not "no social-preview image". Do not add the opt-out checks to `head.ejs`.
- Large-thumbnail resolution is the sibling `large_cover(post)` helper in `scripts/helpers.js` — same "single definition, two call sites" rule as `resolve_cover`. Per-post `cover_large:` (bare boolean, strict identity both ways) overrides the site-wide `cover.large` config in **both** directions; the config key also accepts the string `"true"` for the same reason `cover.fallback` accepts `"false"`. Both partials gate the call on a resolved cover, so a thumbnail-less post never gets the modifier class. The CSS (`.post-grid--list .post-card.post-card--large`, `.pinned-post--large` in `_components.scss`) only *undoes* the side-by-side layout. **The two halves differ in scope**: the card rule is nested under `.post-grid--list`, so it is deliberately inert in grid mode (`grid.columns >= 2`) where cards already stack; `.pinned-post--large` is **not** scoped to the grid — the hero lives outside `.post-grid` and is row-layout at every column count, so it restructures whenever the flag is set. Don't describe the feature as "list view only" without that carve-out. The compound `.post-card.post-card--large` (0,3,0) is what beats `.post-grid--list .post-card` (0,2,0) on `flex-direction` outright; a single class ties it and wins only on source order. (The nested cover rule outranks `.post-grid--list .post-card__cover` either way — the compound is not what saves the cover.)
- Per-post card colours are the `card_color_style(post)` helper in `scripts/helpers.js` — same "single definition, two call sites" rule as `resolve_cover`/`large_cover` (`_partial/post-card.ejs`, `_partial/pinned-post.ejs`). See "Per-post card colours" below.
...
## Per-post card colours

`card_border:` / `card_bg:` front-matter recolour one post's card surface. Resolution is the `card_color_style(post)` helper in `scripts/helpers.js` (next to `resolve_cover`/`large_cover`); it returns the **whole** ` style="…"` attribute (like `category_desc_attr`), so both call sites emit it with `<%- %>`. Call sites (2): `_partial/post-card.ejs`, `_partial/pinned-post.ejs`. Never re-inline the validation in a template.

- **Hex only** — `CARD_HEX_RE` accepts `#rgb`, `#rgba`, `#rrggbb`, `#rrggbbaa` and nothing else. This is deliberately *tighter* than `ICON_COLOR_RE` in `header.ejs`: the feature is specified as hex codes, and the narrow allowlist is what makes raw interpolation into `style="…"` safe (quotes, semicolons and parens can never pass — same posture as the model-viewer `height` allowlist). Do not widen it to keywords or `rgb()`/`oklch()` without revisiting the injection argument.
- **Per-key drop** — an invalid `card_bg` must not discard a valid `card_border`; each key is validated and emitted independently. Non-string values (bare YAML numbers/booleans) are ignored, not stringified.
- **Config-driven colours can't live in compiled SCSS**, so the value arrives as inline CSS custom properties (`--card-border`, `--card-bg`) and `_components.scss` reads them with the tokens as `var()` fallbacks: `var(--card-bg, #{$bg-elevated})`. The `#{}` interpolation is required — a bare `$token` in a `var()` fallback is not reliably substituted. This keeps the "never hardcode a colour" rule intact; same pattern as `--post-grid-cols` and `--compare-pos`.
- **The two roots have different border defaults**: `.post-card` falls back to `$border-subtle`, `.pinned-post` to `$border-focus`. Don't unify them.
- **`.post-card:hover` reads the same custom property** (`border-color: var(--card-border, #{$border-focus})`) so an author's colour survives hover; the box-shadow lift remains the hover cue. Reverting hover to a bare `$border-focus` would snap every custom card back to blue.
- Because `post-card.ejs` is also the related-posts strip in `post.ejs`, the colour follows the post there — intended, not a leak.
- There is **no theme config key and no global kill switch** — the feature is inert unless a post opts in.
...
- Text branch is emitted escaped via `<%= %>` — inline SVG in config is deliberately not supported; ship a file instead. Text icons get `width: auto; height: auto; white-space: nowrap` (`.navbar__logo--text`) so multi-character values widen the box instead of overflowing it. There is deliberately **no `min-width`/fixed height** on the text branch — reserving the 28px image-logo footprint padded a narrow glyph (e.g. a single emoji) with ~6px on each side, making the gap to the title visibly wider than the `icon_after` gap.
- **Text icons live inside `.navbar__brand-text` with the title, not as siblings of the logo** — an `inline-flex` wrapper with `align-items: baseline` and `gap: $sp-2`. Text slots must share the title's baseline: box-centring each slot separately puts a larger icon's baseline *below* the title's, because the baseline sits `(ascent - descent) / 2 × font-size` under the box centre — so a 20px icon next to the 15px title lands ~2px low (and an emoji ~1.5px low). Baseline alignment is font-size independent and needs no per-glyph nudges. **Image** icons stay direct `.navbar__brand` children, centred — an image's baseline is its bottom edge, which would sit the logo on the text baseline.
  - The wrapper carries `navbar__brand-text--icon` only when the *before* icon is text (the one slot that stays visible on mobile); without that modifier it is `display: none` below `$bp-tablet`, because an emptied wrapper would still contribute a flex gap inside `.navbar__brand`.
...
Two independent suppressions, split the same way as the `toc` widget, plus three content overrides:
...
- **Title** — `sidebar.about_title` (default `""`) overrides the widget heading; it lives *inside* the widget (unlike the config gate) because it is content, not a suppression. The value passes through the local `cfgStr` guard, so a bare YAML `false`/number is ignored rather than stringified, and any empty/blank result falls back to `'About'`. Emitted with `<%= %>`. The other four sidebar widgets keep hardcoded headings — only About surfaces user content, so only it needs a user-chosen label.
- **Name and bio** — `sidebar.about_name` / `sidebar.about_description` (both default `""`) override the two body lines, which otherwise come from the *site* config (`author:` and `description:`), not from `theme_config`. Both share one resolution via the local `off()` guard: a bare `false` (or the string `"false"`) suppresses the line outright, a non-blank string overrides it, empty/absent falls back to the site value. The `false` branch is what lets a site drop the name from the sidebar while `author:` stays set for RSS, JSON-LD and the footer — clearing `author:` site-wide would take those with it. Each line is now wrapped in its own `if`, so a suppressed or unset value emits no empty `<p>`.
- **Line breaks in the bio** — `.widget-about__bio` carries `white-space: pre-line` (`_components.scss`), so a multi-line `about_description` (YAML `|` block, or an escaped newline in a double-quoted scalar) breaks where the source does. This is the *only* way to break the line: the value is emitted with `<%= %>` like every other config string, so `<br>` arrives as literal text — do not switch the call site to `<%- %>` to allow markup. `cfgStr` trims the ends, so a `|` block's trailing newline adds no empty last line.
- **Emptiness check** — `_partial/widgets/about.ejs` renders nothing when the **resolved** name and bio, `config.avatar` and both `theme.social` handles are all empty, so a barebones site gets no empty "About" box. It must read the resolved values, not the raw `config.*` ones: with `about_name: false` and nothing else set, a site whose `author:` is filled in would otherwise render a box containing only its heading. Values are counted only when `typeof === 'string'` and non-blank — a bare YAML `false`/`0` is not content (same posture as `cfgStr` in `header.ejs`).
